...
- Per-route `head()` (title, description, og:_, twitter:_). JSON-LD `SoftwareApplication` on `/`, `AboutPage` on `/about`. `robots.txt` + `sitemap.xml` with relative URLs (no project domain yet).
